...
async def test_net_buy_pressure_moves_price_up(mock_db):
    ...
    assert a["prezzo_corrente_crediti"] == pytest.approx(0.02 * 1.020)
...
async def test_no_flow_no_move(mock_db):
    ...
    a = await mock_db.athletes.find_one({"_id": aid})
    assert a["prezzo_corrente_crediti"] == pytest.approx(0.02)
...
async def test_flow_respects_floor(mock_db):
    ...
    await mock_db.athletes.update_one({"_id": aid}, {"$set": {"prezzo_corrente_crediti": 0.00205}})
    now = utc_now()
    ...
    a = await mock_db.athletes.find_one({"_id": aid})
    assert a["prezzo_corrente_crediti"] >= 0.10 * 0.02 - 1e-12  # floor 10% iniziale
