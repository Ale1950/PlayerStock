...
async def main(email: str) -> int:
    ...
    # 1. Wallet mock (€) — fondo €1.000.000, saldo demo dopo qualche scambio
    await db.user_wallets.update_one(
        {"user_id": uid},
        {"$set": {"balance_eur": 758_050.0, "updated_at": now}},
        upsert=True,
    )
    await db.wallet_transactions.insert_one({
        "user_id": uid, "type": "welcome_bonus", "amount": 1_000_000.0,
        "balance_after": 1_000_000.0, "description_it": "Bonus di benvenuto",
    ...
    bal = 1_000_000.0  # D7: fondo €1.000.000 (mock demo; prezzi scenario sono placeholder)
    ...
    await db.user_wallets.update_one(
        {"user_id": uid},
        {"$set": {"balance_eur": round(bal, 4), "updated_at": now}},
    )
